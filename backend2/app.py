...
def listar_tareas():
    return jsonify({"backend": "backend2", "tareas": tareas})

@app.route('/api/tareas/<int:id>', methods=['GET'])
def obtener_tarea(id):
    tarea = next((t for t in tareas if t["id"] == id), None)
    return jsonify({"backend": "backend2", "tarea": tarea})
...
def eliminar_tarea(id):
    ...
    return jsonify({"backend": "backend2", "mensaje": "Eliminada"})
...
def actualizar_tarea(id):
    ...
    for t in tareas:
        if t["id"] == id:
            ...
            return jsonify({"backend": "backend2", "tarea": t})
    return jsonify({"backend": "backend2", "mensaje": "No encontrada"}), 404
